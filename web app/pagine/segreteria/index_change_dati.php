...
<body>
    <?php
        ...
        if (isset($_POST['matricola'])) {
            ...
            $res_verifica = pg_execute($connection, "rep", array($_POST['matricola']));
            $row = pg_fetch_assoc($res_verifica);

            if ($row['is_stud'] == 0) {
                $_SESSION['modifica_stud'] = "L'ID inserito non corrisponde a nessuno studente del sistema!";
                header('Location: ../pagine/segreteria/conf_update_stud2.php');
            }

            echo "<ul class='list-group' id='centrato'>";
            
            echo "<form method='POST' action='../../script/segreteria/conf_update_stud1.php'>";
